...
class Hyperparameters:
    ...
    # --- Depth Recurrence: loop unique layers multiple times for more effective depth ---
    depth_recurrence = int(os.environ.get("DEPTH_RECURRENCE", 2))  # loop layers 2x
    # --- SwiGLU: use gated activation with 2x wider up projection ---
    swiglu_enabled = bool(int(os.environ.get("SWIGLU_ENABLED", "1")))
    # --- Mixed Quantization: use int5 for less sensitive layers, int6 for critical ones ---
...
class MLP(nn.Module):
    ...
    Splits up_w into gate and value projections for better gradient flow.
    Novel modification: SwiGLU replaces LeakyReLU(0.5)² for improved expressivity."""
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
        # up_w is now 2x wider: first half is gate, second half is value
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        hidden = F.linear(x, up_w.to(x.dtype))
        half = hidden.size(-1) // 2
...
class GPT(nn.Module):
    def __init__(
        ...
        self.swiglu_enabled = swiglu_enabled
        ...
        # SwiGLU: up projection is 2x wider (split into gate + value in forward)
        mlp_up_dim = mlp_dim * 2 if swiglu_enabled else mlp_dim
        mlp_down_dim = mlp_dim  # down projection takes half of up (after gating)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        v0 = None
        ve_cache: dict = {}
        # --- Depth Recurrence: loop through all layers multiple times ---
        # Each recurrence pass reuses the same block weights (parameter tying)
        # but the U-Net skip connections only apply on the final pass
        for recurrence_idx in range(self.depth_recurrence):
            skips: list[Tensor] = []
            is_last_pass = (recurrence_idx == self.depth_recurrence - 1)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        v0 = None
        ve_cache: dict = {}
        # --- Depth Recurrence in forward_logits ---
        for recurrence_idx in range(self.depth_recurrence):
            skips: list[Tensor] = []
            is_last_pass = (recurrence_idx == self.depth_recurrence - 1)
            for i in range(self.num_encoder_layers):
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        swiglu_enabled=args.swiglu_enabled,
    ).to(device).bfloat16()
